              ...
              <div className="mt-7 rounded-2xl border border-white/10 bg-white/[.03] p-5">
                ...
                {form.action_type === "coupon" && (
                  <div className="mt-5 grid gap-4 lg:grid-cols-2">
                    ...
                    <label><span className="admin-label">Coupon English highlights (one per line)</span><textarea rows={4} value={joinLines(form.coupon_highlights_en)} onChange={(e)=>setForm(f=>({...f,coupon_highlights_en:lines(e.target.value)}))} className="admin-input"/></label>
                    <label><span className="admin-label">Coupon Somali highlights (one per line)</span><textarea rows={4} value={joinLines(form.coupon_highlights_so)} onChange={(e)=>setForm(f=>({...f,coupon_highlights_so:lines(e.target.value)}))} className="admin-input"/></label>
                  </div>
                )}
              </div>
            )}
