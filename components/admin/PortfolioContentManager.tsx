  ...
  return <section className="admin-content-card mt-8 rounded-3xl border border-white/10 bg-[#170607] p-5 text-white shadow-2xl sm:p-7">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="space-y-5">
        <label className="admin-field block"><span className="text-sm font-semibold text-white">Quote of the Day</span><textarea value={quote} onChange={e=>setQuote(e.target.value)} rows={4} className="admin-input mt-2 w-full rounded-xl border border-white/10 bg-[#240b0e] p-3 text-sm text-white outline-none transition focus:border-[#e45560] focus:ring-2 focus:ring-[#e45560]/20 placeholder:text-white/30" /></label>
        <label className="admin-field block"><span className="text-sm font-semibold text-white">Quote Author</span><input value={author} onChange={e=>setAuthor(e.target.value)} className="admin-input mt-2 w-full rounded-xl border border-white/10 bg-[#240b0e] p-3 text-sm text-white outline-none transition focus:border-[#e45560] focus:ring-2 focus:ring-[#e45560]/20 placeholder:text-white/30" /></label>
      </div>
    </div>
