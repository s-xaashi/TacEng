...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(amount) || amount < 1 || amount > 10000) {
    return NextResponse.json({ error: "Support amount must be between $1 and $10,000." }, { status: 400 });
  }
  if (!body.gateway || !ALLOWED.includes(body.gateway as WalletGateway)) {
    return NextResponse.json({ error: "Unsupported wallet." }, { status: 400 });
  }
  if (!body.account?.trim()) {
    return NextResponse.json({ error: "Phone / account number is required." }, { status: 400 });
  }
  ...
  try {
    ...
    const result = await chargeWallet({
      account: body.account.trim(),
      gateway: body.gateway as WalletGateway,
      amount: amount.toFixed(2),
      currency: "USD",
      order_id: reference,
    });

    const updated = await applySupportVerify(payment.id, {
      sid: result.sid || "",
