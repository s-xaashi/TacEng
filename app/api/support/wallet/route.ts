...
export async function POST(req: NextRequest) {
  let body: {amount?:number; gateway?:string; account?:string};
  ...
  const payment = await createSupportPayment({amount,method:body.gateway,reference,account:body.account.trim()});
  try {
  ...
  } catch (err) { return NextResponse.json({error:err instanceof Error ? err.message : "Payment could not be processed."},{status:502}); }
}