...
import { createSupportPayment, generateSupportReference, validateSupportIdentity } from "@/lib/sifalo/support";
export async function POST(req: NextRequest) {
  let body: { amount?: number; name?: unknown; note?: unknown };
  ...
  try {
    ...
    await createSupportPayment({amount,method:"checkout",reference,name:identity.name,note:identity.note});
  ...
  } catch (err) { return NextResponse.json({error:err instanceof Error ? err.message : "Could not start checkout."},{status:400}); }
}